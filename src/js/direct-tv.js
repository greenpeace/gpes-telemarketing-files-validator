...
/* global $, alert, sha1, tlmkValidator, dtvFileValidator */
		...
		alert("O no has elegido un fichero o tienes una extensión de fichero inválida. Tienes que elegir un fichero .csv");
			...
			config: {
				...
				complete: function(results, file) {
					...
					if ( dtvFileValidator.checkFields( results.meta.fields ) !== true ) {
						$("#errorHeaders").show();
						$("#recordsNumber").text(" ");
						$("#correctHeaders").html( tlmkValidator.arrayToHtmlList( dtvFileValidator.fields ) );
						...
						for (var n in results.data) {
							if (results.data.hasOwnProperty(n)) {
								errorsInRecordS = errorsInRecordS + dtvFileValidator.checkCsvData(n, results.data[n]);										
							}
						}
