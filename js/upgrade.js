...
/* global $, alert, sha1, tlmkValidator, upgradeFileValidator */
    ...
    $('input[type=file]').parse({
      config: {
        ...
        complete: function (results, file) {
          ...
          if (upgradeFileValidator.checkFields(results.meta.fields) !== true) {
            $("#errorHeaders").show();
            $("#recordsNumber").text(" ");
            $("#correctHeaders").html(tlmkValidator.arrayToHtmlList(upgradeFileValidator.fields));
            ...
            for (var n in results.data) {
              if (results.data.hasOwnProperty(n)) {
                errorsInRecordS = errorsInRecordS + upgradeFileValidator.checkCsvData(n, results.data[n]);
              }
            }
