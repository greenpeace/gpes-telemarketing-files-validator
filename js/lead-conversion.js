...
/* global $, alert, sha1, tlmkValidator, leadFileValidator */
    ...
    $('input[type=file]').parse({
      config: {
        ...
        complete: function (results, file) {
          ...
          if (leadFileValidator.checkFields(results.meta.fields) !== true) {
            $("#errorHeaders").show();
            $("#recordsNumber").text(" ");
            $("#correctHeaders").html(tlmkValidator.arrayToHtmlList(leadFileValidator.fields));
            ...
            for (var n in results.data) {
              if (results.data.hasOwnProperty(n)) {
                errorsInRecordS = errorsInRecordS + leadFileValidator.checkCsvData(n, results.data[n]);
              }
            }
