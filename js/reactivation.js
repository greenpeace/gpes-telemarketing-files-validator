...
/* global $, alert, sha1, tlmkValidator, reactivationFileValidator */
    ...
    $('input[type=file]').parse({
      config: {
        ...
        complete: function (results, file) {
          ...
          if (reactivationFileValidator.checkFields(results.meta.fields) !== true) {
            $("#errorHeaders").show();
            $("#recordsNumber").text(" ");
            $("#correctHeaders").html(tlmkValidator.arrayToHtmlList(reactivationFileValidator.fields));
            ...
            for (var n in results.data) {
              if (results.data.hasOwnProperty(n)) {
                errorsInRecordS = errorsInRecordS + reactivationFileValidator.checkCsvData(n, results.data[n]);
              }
            }
