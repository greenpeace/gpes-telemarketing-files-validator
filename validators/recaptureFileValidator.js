recaptureFileValidator = Object.create(tlmkValidator);

recaptureFileValidator.version = "3.0";

recaptureFileValidator.fields = ["DCAMPAIGNMEMBERID", "DINCOMPLETE", "DPROCESSNAME", "AccionGP", "ConfirmacionAccionGP", "REgGivingStatus", "DIDDECONTACTO", "DINITIALSOURCE", "DREGULARGIVING", "Amount", "TransactionsperYear", "CURRENTPAYMENTMETHOD", "SIGNUPDATE", "DWELCOMECALLDATE", "1st payment Amount", "Transaction Payment Method", "Update Payment Details", "AccountName", "IBAN", "DCREDITCARDNUMBER", "DNAMEONCARD", "DEXPIRYMONTH", "DEXPIRYYEAR", "DIATSCUSTOMERCODE", "DIDDECAMPANA", "DLIFETIMEHARDANDSOFTCREDDON", "DLIFETIMEHARDCREDITDONTOTAL", "DLASTYEARSHARDCREDITDONTOTAL", "DFIRSTHARDCREDITDONDATE", "DLASTHARDCREDITDONDATE", "cancelation date", "cancelation MOTIVO", "LRGStatus", "LRGHardFailure", "RazonCancelacion", "ComentariosGuia", "Gender", "DNI", "FirstName", "LastName", "Birthdate", "Street", "PostalCode", "City", "Province", "DCOUNTRY", "PHONE", "MOBILEPHONE", "EMAIL", "MAILOK", "EMAILOK", "SMSOK", "PHONEOK", "PERMITGOVTAXESSENT", "CommunicationPreferenceNotes", "REVCREATECASE", "CASEDESCRIPTION", "CierreGP", "FECHACIERRE", "ID", "PAIS", "Donation Start Date", "UPDATEFREQUENCY", "Amount (OLD)", "TransactionsperYear (OLD)", "SIGNUPDATE (OLD)", "Facturacion TMK"];

recaptureFileValidator.CierreGPValues = [
    ...
    "POSITIVO – Se mantiene como socio",
    "POSITIVO – Se mantiene socio aumenta cuota",
    "POSITIVO – Se mantiene socio reduce cuota",
...
recaptureFileValidator.cancelationMotivo = [
	"Recaptura"
];

recaptureFileValidator.LRGStatus = [
	"Active",
	"Cancelled",
	"Blocked"
];

recaptureFileValidator.checkCsvData = function( rowindex, row ) {
	...
	if ( row["CierreGP"].includes("NEGATIVO") ) {
	...
	} else if ( row["CierreGP"] === "POSITIVO – Se mantiene como socio" ) {
		
		if ( this.isNotEmpty( row["REgGivingStatus"]) ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, REgGivingStatus tiene que estar vacío" + "\n";	
		}
        		
		if ( this.isNotEmpty( row["AccionGP"]) ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, AccionGP tiene que estar vacío" + "\n";	
		}
		
		if ( this.isEmpty( row["TransactionsperYear"]) ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, TransactionsperYear es obligatorio" + "\n";	
		} else if ( row["TransactionsperYear"] == 0) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, TransactionsperYear no puede ser cero" + "\n";
		}
		
		if ( row["CURRENTPAYMENTMETHOD"] !== "Direct Debit" ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, CURRENTPAYMENTMETHOD tiene que ser Direct Debit" + "\n";			
		}
		
		if ( this.isEmpty( row["Amount"]) ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, Amount es obligatorio" + "\n";						
		}
		
		if ( this.isEmpty( row["AccountName"]) ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, AccountName es obligatorio" + "\n";						
		}
		
		if ( this.isEmpty( row["IBAN"]) ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, IBAN es obligatorio" + "\n";						
		}
		
		if ( (Number(row["Amount"]) * Number(row["TransactionsperYear"])) != (Number(row["Amount (OLD)"]) * Number(row["TransactionsperYear (OLD)"]))  ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es Positivo, Amount * TransactionsperYear = Amount(OLD) * TransactionsperYear (OLD)" + "\n";									
		}
		
		if ( row["LRGStatus"] !== "Active" ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, LRGStatus tiene que ser Active" + "\n";			
		}
		
		if ( this.isNotEmpty( row["cancelation MOTIVO"]) ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene como socio, cancelation MOTIVO tiene que estar vacío" + "\n";	
		}
	...
	} else if ( row["CierreGP"] === "POSITIVO – Se mantiene socio aumenta cuota" ) {
	...
	} else if ( row["CierreGP"] === "POSITIVO – Se mantiene socio reduce cuota" ) {
		
        
        
		if ( row["REgGivingStatus"] !== "Downgrade" && row["REgGivingStatus"] !== "Salta"  ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene socio reduce cuota, REgGivingStatus tiene que ser Downgrade o salta" + "\n";			
		}
		
		if ( row["AccionGP"] !== "Downgrade" && row["AccionGP"] !== "Salta" ) {
			errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO – Se mantiene socio reduce cuota, AccionGP tiene que ser Downgrade o salta" + "\n";			
		}
        ...
        if ( this.isFalse(row["REVCREATECASE"]) && row["REgGivingStatus"] !== "Downgrade"  ) {
            errorsInRecord = errorsInRecord + prefix + "Si CierreGP es POSITIVO y REgGivingStatus es salta  – Se mantiene socio reduce cuota, REVCREATECASE tiene que ser true" + " (CASEDESCRIPTION)" + "\n";
        }
