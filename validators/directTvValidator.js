...
dtvFileValidator.CierreGPValues =[
	"Positivo",
...
dtvFileValidator.checkCsvData = function( rowindex, row ) {
	...
	if ( this.isNotEmpty(row["DCAMPAIGNMEMBERID"]) ) {
		errorsInRecord = errorsInRecord + prefix + this.errors.isNotEmpty + " (DCAMPAIGNMEMBERID)" + "\n";
	}
	...
	if ( this.isNotEmpty(row["LeadStatus"]) ) {
		errorsInRecord = errorsInRecord + prefix + this.errors.isNotEmpty + " (LeadStatus)" + "\n";
	}
	
	if ( !this.isEmpty(row["DLEADID"]) && row["DLEADID"].length != 18 ) {
	...
	if ( this.isNotEmpty(row["Donation Start Date"]) && this.isValidDate(row["Donation Start Date"]) ) {
		...
		if ( thisMonth <= 11) {
			...
			if ( Number(month) != 1 ) {
				errorsInRecord = errorsInRecord + prefix + "El valor del mes tiene que corresponder a enero" + " (Donation Start Date)" + "\n";	
			}
